...
# sed -i -r "s/debian/$db_name/1; s/user/$db_user/1; s/pwd/$db_pwd/1" wp-config.php
# wp core install --url=$DOMAIN_NAME/ --title=$WP_TITLE --admin_user=$WP_ADMIN_USR --admin_password=$WP_ADMIN_PWD --admin_email=$WP_ADMIN_EMAIL --skip-email --allow-root --prompt=db
# wp user create $WP_USR $WP_EMAIL --role=author --user_pass=$WP_PWD --allow-root
